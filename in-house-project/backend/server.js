...
// =================================================
// STUDENT COMPLAINTS (WITH HOSTEL)
// =================================================
...
app.get("/complaints/warden/:email", (req, res) => {
  ...
  db.query(hostelSql, [email], (err, hostels) => {
    if (err || hostels.length === 0) return res.json([]);
    ...
    let sql = `
      ...
      JOIN complaint_categories cc ON c.category_id = cc.id
      WHERE ch.hostel_id IN (${placeholders})   
    `;
  ...
  params.push(parseInt(category));  
    ...
    db.query(sql, params, (err2, complaints) => {
      if (err2) {
        console.log("Fetch warden complaints error:", err2);
        return res.json([]);
      }
...
app.get("/warden/students/:email", async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Get hostel_ids assigned to this warden
    const [hostels] = await db.promise().query(
      "SELECT hostel_id FROM warden_hostel WHERE warden_id = ?",
      [warden_id]
    );

    if (hostels.length === 0) return res.json([]);

    const hostelIds = hostels.map(h => h.hostel_id);

    // 3️⃣ Get students using course_hostel mapping
    const [students] = await db.promise().query(
      `SELECT s.student_id, s.name
       FROM student s
       JOIN course_hostel ch ON s.course_id = ch.course_id
       WHERE ch.hostel_id IN (?) 
...
app.get("/api/attendance/:date/:email", (req, res) => {
  const { date, email } = req.params;

  // 1) Find warden by email and get hostel_id + warden_id
  const getWardenQuery = `
    SELECT warden_id, hostel_id
    FROM warden
    WHERE email = ?
    LIMIT 1
  `;
  ...
  db.query(getWardenQuery, [email], (err, wardenResult) => {
    ...
    // 2) Get all students of that hostel + attendance status for selected date
    const getStudentsAttendanceQuery = `
      ...
      LEFT JOIN attendance a 
        ...
        AND a.date = ?
      WHERE s.hostel_id = ?
      ORDER BY s.room_no ASC, s.name ASC
    `;
...
app.post("/api/attendance/mark", (req, res) => {
  ...
  // Get warden_email from first record
  ...
  // 1) Find warden_id using warden_email
  const getWardenQuery = `
    SELECT warden_id
    FROM warden
    WHERE email = ?
  ...
  db.query(getWardenQuery, [wardenEmail], (err, wardenResult) => {
    ...
    // 3) Insert attendance with marked_by
...
app.get("/api/warden/students/:email", (req, res) => {
  const { email } = req.params;
...
app.get("/api/attendance-records/:email", (req, res) => {
  const { email } = req.params;

  // 1) Find warden details
  const getWardenQuery = `
    SELECT warden_id, hostel_id
    FROM warden
    WHERE email = ?
  ...
  db.query(getWardenQuery, [email], (err, wardenResult) => {
    ...
    // 2) Get attendance records of only students from that hostel
    const getAttendanceRecordsQuery = `
      ...
      WHERE s.hostel_id = ?
      ORDER BY a.date DESC, s.room_no ASC, s.name ASC
    `;
